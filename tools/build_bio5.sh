...
PART="$1"
P=projects/bio-c5-la-03
CLIP=inbox/bio5/part${PART}.mp4
SP="${SCRATCH:-/tmp}/bio5_${PART}"; mkdir -p "$SP"
...
BODY="tools/edu/renderer/out/bio5_${PART}_body.mp4"
if [ -f "$BODY" ] && [ "$BODY" -nt "tools/edu/renderer/spec/_bio5_${PART}.json" ] \
...
( cd tools/edu/renderer && cp spec/_bio5_${PART}.json spec/segments.json && \
  nice -n 10 env BG=assets/biology-background.png \
       OUT=out/bio5_${PART}_body.mp4 node full.mjs ) >"$SP/body.log" 2>&1 \
...
if [ -f "$CARD" ] && [ "$CARD" -nt "$P/meta.json" ] \
  ...
  log "question card"
  ( cd "$P/manim_code" && PYQ_PROJECT=bio-c5-la-03 PYQ_PART=$PART \
...
log "background"
printf "file '%s'\nfile '%s'\n" "$SP/card.mp4" \
  "$PWD/tools/edu/renderer/out/bio5_${PART}_body.mp4" > "$SP/list"
...
log "compositing"
OUT="$P/final/bio5_part${PART}.mp4"; mkdir -p "$P/final"
...
# The graphics here are ANIMATIONS, not chips: full-frame transparent renders
# that cannot be raised, only scaled. fit_clips measures the presenter and
# solves for the scale that clears him.
log "fitting animations above the presenter"
$PY tools/fit_clips.py "$SP/prechip.mp4" "$SP/bg.mp4" \
  "$P/plan_part${PART}.json" | tee "$SP/fit.log" \
  || { log "!! fitting failed"; exit 1; }
...
$PY tools/dash_gate.py "$P" - "$P/plan_part${PART}.json" \
  || { log "!! dash gate failed - refusing to burn"; exit 1; }

log "animations"
nice -n 10 $PY tools/burn_clips.py "$SP/prechip.mp4" "$OUT" "$P/plan_part${PART}.json" \
  >"$SP/clips.log" 2>&1 || { log "!! animations failed — $SP/clips.log"; exit 1; }
