...
for i, s in enumerate(S):
    ...
    # A spec built from the presenter's own word timings does not get to choose
    # how long a phrase is on screen or how the phrases are spaced -- the
    # recording decides both. The 2.5s floor, the 0.2s beat between phrases and
    # the words-per-second band are all AUTHORING rules; applying them to a
    # transcript-timed spec would mean pushing captions off the words they
    # belong to. Enforcing the 0.2s gap is in fact what put a blank caption band
    # between every pair of lines. Everything else below still applies.
    timed = s.get("timing") == "transcript"

    for p in ph:
        d = round(p["t_out"] - p["t_in"], 2)
        if d < 2.5 and not timed:
            err.append(f"seg{sid}: phrase {d}s < 2.5s  '{p['text'][:34]}'")
        if timed and d < 0.6:
            err.append(f"seg{sid}: phrase only {d}s on screen  '{p['text'][:34]}'")
    ...
    for a, b in zip(ph, ph[1:]):
        ...
        want = 0.0 if timed else 0.2
        if abs(gap - want) > 0.001:
            err.append(f"seg{sid}: gap {gap}s between phrases (need exactly {want})")
    ...
    if not timed and not (1.5 <= wps <= 2.7):
