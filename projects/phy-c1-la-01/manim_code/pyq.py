...
# shrinks from here automatically when a line is long — see MAX_CAPTION_LINES.
CAPTION_SIZE = 55
MAX_CAPTION_LINES = 2      # a caption reads as a punchy line or two, not a paragraph
CAPTION_MIN_SIZE = 32      # legibility floor on a phone screen
...
class PyqPart(ThemedScene):
    ...
    def caption(self, text, size=CAPTION_SIZE):
        ...
        wraps to more than MAX_CAPTION_LINES lines.

        CAPTION_SIZE is tuned against TTS-engineered lines, written short on
        purpose. A project driven by a REAL recorded clip has no such control
        over sentence length — phy-c1-la-01's part 2 averaged 15 words a
        caption against part 1's 7.6, and a 55pt caption wrapped to 4 lines
        sat on top of the formula beneath it. Shrinking automatically means no
        project needs its own tuned constant; a short line still renders at
        the full, tuned size exactly as before.
        """
    ...
    def construct(self):
        ...
        if not no_card:
            # `sheet` is the photographed exam-paper corner from
            # tools/paper_header.py, generated per project. Not every project has
            # gone through that (font/asset dependencies live only on the machine
            # that built it), so fall back to the plain lined notepaper the card
            # already knows how to typeset onto rather than crashing the render.
                ...
                if at == i:
                    ...
                    if gone:
                        self._layout_violations.append(
                            f"t={self.renderer.time:6.2f}s REVEALED ITEM VANISHED: {gone}")
    ...
    def at(self, t):
        ...
        `self.renderer.time` is Manim's own running total, which is what makes the
